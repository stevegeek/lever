...
| `trust_forwarded_host` | bool | no | `false` | Make the proxy's `Host` check (its DNS-rebinding defence) read `X-Forwarded-Host`, when a request carries exactly one, instead of `Host` — for a front that rewrites `Host` to a name the default refuses. **Off by default**: any client that reaches the listener directly can set that header, so turning it on leaves the rebinding defence to reachability alone. Warned about like a non-loopback `bind`. Not needed when the front sends `Host` as `base_url`'s host, or as the address it dials (`127.0.0.1:<port>`, or `<bind>:<port>`). |
