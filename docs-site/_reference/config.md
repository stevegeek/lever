...
| `trust_forwarded_host` | bool | no | `false` | Make the proxy's `Host` check (its DNS-rebinding defence) read `X-Forwarded-Host`, when a request carries exactly one and its `Host` is an IP address, instead of `Host` — for a front that rewrites `Host` to an IP address the default refuses. A rebind sends a name in `Host`, so it is still refused. **Off by default**: anything that reaches the listener directly can set that header. Warned about like a non-loopback `bind`. Not needed when the front sends `Host` as `base_url`'s host, or as the address it dials (`127.0.0.1:<port>`, or `<bind>:<port>`). |
