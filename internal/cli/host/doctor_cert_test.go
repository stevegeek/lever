...
func writeBrokerLog(t *testing.T, st state.State, content string) {
	t.Helper()
...
func TestCheckAgentCert_NoLogFilePasses(t *testing.T) {
	now := time.Now()
	st := state.State{Dir: t.TempDir()} // no broker.out.log written
	if r := checkAgentCert(st, now); !r.ok {
...
func stampBrokerPID(t *testing.T, st state.State, started time.Time) {
	t.Helper()
