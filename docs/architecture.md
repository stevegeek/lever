...
> **Designed, not yet built.** This describes the intended architecture of the Go `lever` core,
> which is in progress. The containment *primitives* are hand-validated (see
> [security-model.md](security-model.md)); the integrated runtime behaviours below are not yet
> shipped. Present-tense descriptions are design intent.
...
terms recur below: the **broker** (Scion's host-side component that asks Docker to create containers
and apply mounts) and the **hub** (Scion's registry of projects and agents). Lever adds three things
Scion does not: an **opinionated project model** (a project is a directory), a **security jail**
that contains the whole runtime, and a **single operator surface** (`lever`).
...
- **Only the OrbStack VM is a hardware-virtualization boundary** — you already run it for all Docker
  ...
  [security-model.md §5](security-model.md).)
...
- **Overlapping mounts are intentional** — the manager's workspace physically contains the grove
  ...
  *between* the manager and groves currently rests on convention, not enforcement, and is hardened
  by the planned inner auth layer ([security-model.md §4](security-model.md)). It is **not** a
...
| Scion server + broker | container lifecycle, sessions, attach/resume, typed messaging | core (runs inside the jail) |
| rootless dockerd | the container runtime the broker drives (rootless — see security-model.md) | core (inside the jail) |
...
[security-model.md §2.3](security-model.md) for the rootless requirement.
