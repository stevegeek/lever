...
func TestTypesKeepTheirWireShape(t *testing.T) {
	cases := []struct {
		...
		{"WorkerTicketRequest", WorkerTicketRequest{Worker: "w"}, `{"worker":"w"}`},
		{"WorkerTicketResponse", WorkerTicketResponse{Worker: "w", Path: "/run/user/1/lever/tickets/w/bootstrap.json"}, `{"worker":"w","path":"/run/user/1/lever/tickets/w/bootstrap.json"}`},
